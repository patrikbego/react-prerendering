...
- 8 basic data types (dynamically typed)
    - `Number == 12.123, Infinity, -Infinity & NaN`   
    (both integer and floating point) (2^53-1) (that's 9007199254740991), or less than -(2^53-1)`
...
- Functions provide a built-in method bind that allows to fix this.
	- same can be achieved with the wrapper for e.g.:  
	 `setTimeout(() => user.sayHi(), 1000); // Hello, John!` `let boundFunc = <some function>.bind(context);`
- Method `<some function>.bind(context, ...args)` returns a "bound variant" of function func that fixes the context this and first arguments if given.
